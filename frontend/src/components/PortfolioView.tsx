...
interface PortfolioViewProps {
  ...
  onOpenAddModal: () => void;
  onOpenAdjustModal: (ev: PositionEvaluation) => void;
                    ...
                    >
                      ...
                      <td className="py-3.5 px-3">
                        ...
                        {ev.isLive ? (
                          ...
                          <span className="text-[9px] text-slate-500 font-mono">CACHED</span>
                        )}
                      </td>
                      <td className="py-3.5 px-3 text-right text-slate-300 font-bold">{ev.quantity}</td>
                      ...
                      <td className="py-3.5 px-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onOpenAdjustModal(ev);
                            }}
                            className="px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 text-cyan-400 font-sans text-[11px] font-medium border border-slate-700 transition flex items-center gap-1"
                            title="Add or Reduce shares"
                          >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div>
                  ...
                  <span className="text-xs text-slate-400">Position Health & Decision Logic</span>
                </div>
                {getActionBadge(selectedEvaluation.action)}
              </div>

              {/* Adjust / Pyramid Button */}
              <button
                onClick={() => onOpenAdjustModal(selectedEvaluation)}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-medium text-xs bg-slate-800 hover:bg-slate-750 text-cyan-300 border border-slate-700 transition"
              >
                <SlidersHorizontal className="w-3.5 h-3.5 text-cyan-400" />
                <span>Adjust Position (Add / Trim Shares)</span>
              </button>
